...
gdjs.evtsExt__Tween3D__DefineHelperClasses.userFunc0x10ecfa0 = function GDJSInlineCode(runtimeScene, eventsFunctionContext) {
"use strict";
...
class Tint {
  ...
  setColor(tintColor, originalTint) {
    ...
    for (const [materialId, material] of this.materials) {
       const originalColor = originalMaterials.get(materialId).color;
  ...
  createTint(object, color) {
    ...
    if (this.recycledTints.length > 0) {
      ...
      object.get3DRendererObject().traverse((mesh) => {
        if (mesh.material && mesh.material.color) {
          const material = mesh.material;
          const originalId = material.id;
          if (materials.has(originalId)) {
            // Meshes can share the same material.
            return;
          }
          const tintedMaterial = mesh.material.clone();
...
gdjs.evtsExt__Tween3D__DefineHelperClasses.userFunc0x10ecfa0(runtimeScene, typeof eventsFunctionContext !== 'undefined' ? eventsFunctionContext : undefined);
